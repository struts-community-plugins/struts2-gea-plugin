package com.gruuf.struts2.gae.interceptor;
...
public class GaeFileUploadInterceptor extends AbstractInterceptor {
        ...
        while (fileParameterNames != null && fileParameterNames.hasMoreElements()) {
            ...
            if (isNonEmpty(contentType)) {
                ...
                if (isNonEmpty(fileName)) {
                    ...
                    List<GaeUploadedFile> fileItemStreams = multiWrapper.getFileContents(inputName);
                    // get a File object for the uploaded File
                    if (fileItemStreams != null && fileItemStreams.size() > 0) {
                        List<GaeUploadedFile> acceptedFiles = new ArrayList<>(fileItemStreams.size());
                        List<String> acceptedContentTypes = new ArrayList<>(fileItemStreams.size());
                        List<String> acceptedFileNames = new ArrayList<>(fileItemStreams.size());
                        String contentTypeName = inputName + "ContentType";
                        String fileNameName = inputName + "FileName";

                        for (int index = 0; index < fileItemStreams.size(); index++) {
                            if (acceptFile(action, fileItemStreams.get(index), fileName[index], contentType[index], inputName, validation, ac.getLocale())) {
                                acceptedFiles.add((fileItemStreams.get(index)));
                        ...
                        if (!acceptedFiles.isEmpty()) {
                            ...
                            newParams.put(inputName, new Parameter.File(inputName, acceptedFiles.toArray(new GaeUploadedFile[acceptedFiles.size()])));
     ...
     * @throws IOException
     */
    protected boolean acceptFile(Object action, GaeUploadedFile file, String filename, String contentType, String inputName, ValidationAware validation, Locale locale) throws IOException {
        ...
        if (file == null) {
            ...
            LOG.error(errMsg);
        } else if (maximumSize != null && maximumSize < file.getLength()) {
            String errMsg = getTextMessage(action, "struts.messages.error.file.too.large", new Object[]{inputName, filename, "" + file.getLength()}, locale);
            if (validation != null) {
                validation.addFieldError(inputName, errMsg);
            }
